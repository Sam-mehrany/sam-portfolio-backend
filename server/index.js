...
const upload = multer({
  storage: storage,
  limits: {
    fileSize: 500 * 1024 * 1024 // 500MB limit for images and videos
  },
...
db.serialize(() => {
  ...
  // Migration: Add videos column if it doesn't exist
  // Check if the column exists first by querying table info
    ...
    if (!hasVideosColumn) {
      ...
      db.run(`ALTER TABLE projects ADD COLUMN videos TEXT`, (err) => {
        if (err) {
          ...
          console.log('✅ Successfully added videos column to projects table');
        }
      });
    } else {
...
app.post('/api/projects', protectRoute, (req, res) => {
  const { slug, title, year, blurb, tags, thumbnail, images, videos, outcome, challenge, solution, content } = req.body;
  const sql = `INSERT INTO projects (slug, title, year, blurb, tags, thumbnail, images, videos, outcome, challenge, solution, content) VALUES (?,?,?,?,?,?,?,?,?,?,?,?)`;
  const params = [slug, title, year, blurb, JSON.stringify(tags), thumbnail, JSON.stringify(images), JSON.stringify(videos || []), outcome, challenge, solution, JSON.stringify(content)];
  db.run(sql, params, function(err) {
    if (err) return res.status(500).json({ "error": err.message });
    res.status(201).json({ "data": { id: this.lastID } });
  });
...
app.put('/api/projects/:id', protectRoute, (req, res) => {
  const { slug, title, year, blurb, tags, thumbnail, images, videos, outcome, challenge, solution, content } = req.body;
  const sql = `UPDATE projects SET slug = ?, title = ?, year = ?, blurb = ?, tags = ?, thumbnail = ?, images = ?, videos = ?, outcome = ?, challenge = ?, solution = ?, content = ? WHERE id = ?`;
  const params = [slug, title, year, blurb, JSON.stringify(tags), thumbnail, JSON.stringify(images), JSON.stringify(videos || []), outcome, challenge, solution, JSON.stringify(content), req.params.id];
  db.run(sql, params, function(err) {
    if (err) return res.status(500).json({ "error": err.message });
    res.json({ message: "updated", changes: this.changes });
  });
